...
def ReadTxtNet(file_path="", undirected=True):
    ...
    if file_path == 'youtube' or file_path == 'blog':
        ...
        download(_get_dgl_url(os.path.join('dataset/DeepWalk/',
                 '{}.zip'.format(file_path))), path=zip_file_path)
    ...
    with open(file_path, "r") as f:
        for line in f.readlines():
            ...
            assert len(tup) in [
                2, 3], "The format of network file is unrecognizable."
            if len(tup) == 3:
                n1, n2, w = tup
            elif len(tup) == 2:
                n1, n2 = tup
...
class DeepwalkDataset:
    def __init__(self,
        ...
        if load_from_ogbl:
            assert len(
                gpus) == 1, "ogb.linkproppred is not compatible with multi-gpu training (CUDA error)."
            from load_dataset import load_from_ogbl_with_name
            self.G = load_from_ogbl_with_name(ogbl_name)
            self.G = make_undirected(self.G)
        else:
            self.net, self.node2id, self.id2node, self.sm = ReadTxtNet(
                net_file)
        ...
        self.seeds = torch.split(shuffle_walks(seeds),
                                 int(np.ceil(len(self.valid_seeds) *
                                     self.num_walks / self.num_procs)),
                                 0)
        end = time.time()
        t = end - start
...
class DeepwalkSampler(object):
    ...
    def sample(self, seeds):
        walks = dgl.sampling.random_walk(
            self.G, seeds, length=self.walk_length-1)[0]
        return walks
