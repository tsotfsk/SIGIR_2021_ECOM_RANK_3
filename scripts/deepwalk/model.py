...
def async_update(num_threads, model, queue):
    ...
    while True:
        ...
        if grad_u is None:
            return
        with torch.no_grad():
            model.u_embeddings.weight.data.index_add_(
                0, nodes.view(-1), grad_u)
            model.v_embeddings.weight.data.index_add_(
                0, nodes.view(-1), grad_v)
...
class SkipGramModel(nn.Module):
    ...
    def create_async_update(self):
        ...
        self.async_p = mp.Process(target=async_update, args=(
            self.num_threads, self, self.async_q))
        self.async_p.start()
    ...
    def fast_learn(self, batch_walks, neg_nodes=None):
        ...
        emb_u = self.u_embeddings(
            nodes).view(-1, self.emb_dimension).to(self.device)
        emb_v = self.v_embeddings(
            nodes).view(-1, self.emb_dimension).to(self.device)
        ...
        if self.record_loss:
            self.loss.append(torch.mean(
                self.fast_logsigmoid(pos_score)).item())

        # [batch_size * num_pos, dim]
        if self.lap_norm > 0:
            grad_u_pos = score * emb_pos_v + \
                self.lap_norm * (emb_pos_v - emb_pos_u)
            grad_v_pos = score * emb_pos_u + \
                self.lap_norm * (emb_pos_u - emb_pos_v)
        else:
            grad_u_pos = score * emb_pos_v
            grad_v_pos = score * emb_pos_u
        ...
        if self.record_loss:
            self.loss.append(self.negative * self.neg_weight *
        ...
        # use adam optimizer
        grad_u = adam(grad_u, self.state_sum_u, nodes,
                      lr, self.device, self.only_gpu)
        grad_v = adam(grad_v, self.state_sum_v, nodes,
                      lr, self.device, self.only_gpu)
        if neg_nodes is not None:
            grad_v_neg = adam(grad_v_neg, self.state_sum_v,
        ...
        if self.mixed_train:
            ...
            if self.async_update:
                ...
                self.async_q.put(
                    (grad_u, grad_v, grad_v_neg, nodes, neg_nodes))
    ...
    def save_embedding(self, dataset, file_name):
        ...
        if self.norm:
            embedding /= np.sqrt(np.sum(embedding *
    ...
    def save_embedding_pt(self, dataset, file_name):
        ...
        try:
            ...
            if self.norm:
                embedding /= torch.sqrt(torch.sum(embedding.mul(embedding),
                                        1) + 1e-6).unsqueeze(1)
